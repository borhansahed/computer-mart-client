...
const Payment = () => {
    const {id} = useParams();
    const url = `https://computer-mart-server.vercel.app/booking/${id}`
  ...
  <div className="card-body">
    <h2 className=" text-xl text-yellow-500">Hello Dear   {booking?.customerName ? booking.customerName  : <span className=''>Customer</span>} ,</h2>
  { !booking.paid ? <h2 className=" text-2xl font-bold">Please pay for <span className=''>{booking.product}</span> </h2> : <p className='text-2xl'>Congratulations</p>}
   { !booking.paid ? <h2 className=" text-xl ">Your <span className='text-2xl'>{booking.quantity}</span> <span className='text-yellow-500'>{booking.product}</span> is waiting for you</h2> : <h2 className=" text-xl ">Your <span className='text-2xl'>{booking.quantity}</span> <span className='text-yellow-500'>{booking.product}</span>  is going under shipping</h2> }
