...
const MyOrders = () => {
  ...
  useEffect(() => {
    if (user) {
      fetch(
        `https://computer-mart2.onrender.com/booking?customer=${user.email}`
      )
        .then((res) => res.json())
  ...
  return (
    <div>
      <h3 className="text-2xl lg:text-3xl mt-4 text-yellow-500 mb-3">
        {" "}
        Your Orders
      </h3>
        ...
        <table className="table w-full">
          <thead>
            <tr className="text-yellow-500">
              ...
              <th className="text-base lg:text-xl font-medium">
                ...
                <td>
                  {order.price && !order.paid && (
                    <Link to={`/dashboard/payment/${order._id}`}>
                      <button className="btn btn-outline btn-sm btn-warning text-base px-5">
                        Pay
                      </button>
                    </Link>
                  )}
                  {order.price && order.paid && !order.shipment && (
                    <button
                      onClick={payment}
                      className=" text-base px-5 btn-sm  btn btn-outline btn-success"
                    >
                      Paid
                    </button>
                  )}
                  {!order.paid && (
                    <button
                      ...
                      className="btn btn-outline btn-sm btn-error text-base ml-3"
                    >
                      Cancel
                    </button>
                  )}
                  {order.paid && !order.shipment && (
                    <button
                      onClick={pending}
                      className="btn btn-outline btn-sm btn-warning text-base ml-3"
                    >
                      Pending
                    </button>
                  )}
